    ...
    while retry_count > 0:
        try:
           # html = requests.get('http://demo.spiderpy.cn/',headers = headers.get('naive_header'),timeout=3,proxies={"http": "http://{}".format(proxy)})
            ...
            #html = requests.get(url=url,headers = headers.get('naive_header'),timeout=1, )
            # 使用代理访问
            #print('bar')
            return html
        except Exception:
...
def downloadVideo (pool,path,res):
    ...
    try :
        ...
        r.hset(path+"badvideo",title,video_url)
        raise
...
def get_all_video_form_homepage(url): 
    ...
    #print(len(href))
        #获得用户总视频数
    try:
        video_num = driver.find_elements(by= By.CSS_SELECTOR, value=" #douyin-right-container > div > div > div > div:nth-child(2) > div:nth-child(1) > div > div > div > div > h2 > span:nth-child(2)")[0]
        print(int(video_num.get_attribute('innerHTML')))
        video_num = int(video_num.get_attribute('innerHTML'))
    except:
        video_num = len(href)
    driver.quit()
    return href,video_num
    #r = redis.Redis(connection_pool=pool)

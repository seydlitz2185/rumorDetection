...
def load_dataset(indexs,sens, pad_size=32):
    ...
    for i in range(len(sens)):
        ...
        elif model_name == 'FastText_gp':
            index = indexs[i]
            sentiment = [data.loc[index,'sentiments']]
            length = [data.loc[index,'length']]
            cos_sim  = [data.loc[index,'cos_sim']]
            topic = eval(data.loc[index,'pca_topic_vectors'])
            buckets = config.n_gram_vocab
            bigram = []
            trigram = []
            # ------ngram------
